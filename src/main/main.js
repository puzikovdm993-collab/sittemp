...
/**
 * Загрузка проекта с сервера (MinIO)
 * @param {string} projectId - ID проекта
 * @returns {Promise<object>} Данные проекта
 */
function fetchProjectFromMinIO(projectId) {
    ...
    return fetch(`${API_BASE_URL}/load_project/${projectId}`)
        ...
        .catch(error => {
            console.error("Ошибка загрузки проекта из MinIO:", error);
            throw error; // Пробрасываем ошибку для обработки вызывающим кодом
        });
}

/**
 * Загрузка проекта с синхронизацией через IndexedDB
...
async function loadProject(projectId) {
    try {
        // Используем синхронизацию через ProjectDB
        // Сначала пробуем загрузить из IndexedDB, если нет - загружаем из MinIO и кэшируем
        const projectData = await ProjectDB.syncProject(projectId, fetchProjectFromMinIO);
        return projectData;
    } catch (error) {
        console.error("Не удалось загрузить проект через синхронизацию:", error);
        throw error;
    }
}
